...
def create_pdf():
    ...
    pdf.cell(200, 10, txt="Courses Evaluation Dashboard", ln=True, align='C')
    
    # Ngày cập nhật
    pdf.set_font('Arial', '', 12)
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Last updated by: {box_date}", ln=True, align='C')

    # Bảng kết quả đánh giá
    pdf.ln(10)
    pdf.set_font('Arial', '', 12)
    for index, row in result_df.iterrows():
        # Tạo một dòng cho mỗi câu hỏi và đánh giá trung bình
        pdf.cell(200, 10, txt=f"{row['Câu hỏi']}: {row['Đánh giá trung bình']}", ln=True)
    
    # Lưu file PDF vào thư mục hiện tại (hoặc thư mục Downloads)
    output_path = os.path.expanduser('~/Downloads/evaluation_report.pdf')
...
if st.button("Tải báo cáo PDF"):
    ...
    st.markdown(f"[Tải về PDF](file://{output_file})")